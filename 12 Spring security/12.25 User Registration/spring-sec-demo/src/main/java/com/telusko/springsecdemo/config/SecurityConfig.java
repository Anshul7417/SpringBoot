...
public class SecurityConfig {
	...
	@Autowired
	private UserDetailsService userDetailsService;       // it is a interface which we havec to implement

	@Bean
	public AuthenticationProvider authProvider() {
		DaoAuthenticationProvider provider = new DaoAuthenticationProvider();
		provider.setUserDetailsService(userDetailsService);
		provider.setPasswordEncoder(NoOpPasswordEncoder.getInstance());      // it means we are not encoding the password till now
		return provider;
	}
